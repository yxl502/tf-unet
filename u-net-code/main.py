...
myGene = trainGenerator(2, 'data/road/train', 'image', 'label', data_gen_args, save_to_dir=None)

model_use = unet()#初始化unet模型
#保存模型
model_checkpoint = ModelCheckpoint('unet_road.hdf5', monitor='loss', verbose=1, save_best_only=True)
...
model_use.fit_generator(myGene, steps_per_epoch=150, epochs=10, callbacks=[model_checkpoint])
# 训练函数
...
testGene = testGenerator("data/road/test")
# print(testGene)
...
#保存测试结果
saveResult("data/road/test", results)
print('------------------')